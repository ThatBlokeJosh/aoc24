            ...
            match field {
                ...
                _ => {
                    ...
                    match antennas.get_mut(field) {
                        Some(v) => {v.push(Coordinate{x: j as i32, y: i as i32});}
                        None => {
                            antennas.insert(*field, vec![Coordinate{x: j as i32, y: i as i32}]);
                        } 
                    }
                }
            }
    ...
    for (l, coords) in antennas {
        for (i, antenna) in coords.iter().enumerate() {
            for j in i+1..coords.len() {
                ...
                let diff1 = Coordinate{x: antenna.x + (antenna.x - pair.x), y: antenna.y + (antenna.y - pair.y)};

                match grid.get(diff1.y, diff1.x) {
                    Some(_b) => {
                ...
                let diff2 = Coordinate{x: pair.x + (pair.x - antenna.x), y: pair.y + (pair.y - antenna.y)};

                match grid.get(diff2.y, diff2.x) {
                    Some(_b) => {
    ...
    for (l, coords) in antennas {
        for (i, antenna) in coords.iter().enumerate() {
            ...
            for j in i+1..coords.len() {
                ...
                let diff1 = Coordinate{x: antenna.x - pair.x, y: antenna.y - pair.y};
                let mut t = *antenna;

                'a: loop {
                    let antinode = Coordinate{x: t.x + diff1.x, y: t.y + diff1.y};
                ...
                let diff2 = Coordinate{x: pair.x - antenna.x, y: pair.y - antenna.y};
                let mut t2 = pair;

                'b: loop {
                    let antinode = Coordinate{x: t2.x + diff2.x, y: t2.y + diff2.y};
                    match grid.get(antinode.y, antinode.x) {
                        Some(_b) => {
                            counter.insert((antinode.x, antinode.y));
                        },
