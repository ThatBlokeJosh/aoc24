...
impl Operator {
    fn new(opcode: u64, operand: u64, reg: Registers) -> Operator {
        let mut op = Operator{literal: 0, combo: 0, opcode: Opcode::Adv};    
        match opcode {
        ...
        match operand {
            1 => {op.literal = 1; op.combo = 1;}
            2 => {op.literal = 2; op.combo = 2;}
            3 => {op.literal = 3; op.combo = 3;}
            4 => {op.literal = 4; op.combo = reg.a;}
            5 => {op.literal = 5; op.combo = reg.b;}
            6 => {op.literal = 6; op.combo = reg.c;}
            7 => {op.literal = 7}
            _ => {}
        }
        return op;
    ...
    fn eval(&mut self, reg: &mut Registers) -> bool {
        ...
        match self.opcode {
            Opcode::Adv => {
                reg.a = reg.a >> self.combo; 
            } 
            Opcode::Bxl => {
                reg.b = reg.b ^ self.literal;
            } 
            Opcode::Bst => {
                reg.b = self.combo % 8;
            } 
...
pub fn part1() -> std::io::Result<()> {
    ...
    for line in reader.lines()  {
        ...
        if !parse_reg {
            reg = Registers{a: reg_nums[0], b: reg_nums[1], c: reg_nums[2]};
...
pub fn part2() -> std::io::Result<()> {
    ...
    for depth in 0..program.len() {
        ...
        for candidate in candidates.iter_mut() {
            let a = *candidate * 8; 
            for b in 0..8 {
                let mut r = Registers{a: a + b, b: 0, c: 0};
