...
fn push(direction: &Direction, coords: &Cartesian, grid: &mut Grid<Entry>) -> bool {
    ...
    if let Some(e) = grid.get(lookahead.y, lookahead.x) {
        match e {
            Entry::Wall => {
                return false;
            }
            Entry::Box => {
                if push(direction, &lookahead, grid) {
                    grid.swap(
                        (
                            (lookahead.y + val.1).try_into().unwrap(),
                            (lookahead.x + val.0).try_into().unwrap(),
                        ),
                        (
                            lookahead.y.try_into().unwrap(),
                            lookahead.x.try_into().unwrap(),
                        ),
                    );
                    return true;
                } else {
        ...
        match e {
            ...
            Entry2::BoxLeft => {
                if push2(direction, &lookahead, grid, false) {
                    ...
                    if val.1 != 0 && !ignore && !push2(direction, &right, grid, true) {
                        return false;
                    }
                    return true;
                } else {
            ...
            Entry2::BoxRight => {
                if push2(direction, &lookahead, grid, false) {
                    ...
                    if val.1 != 0 && !ignore && !push2(direction, &left, grid, true) {
                        return false;
                    }
                    return true;
                } else {
...
pub fn part2() -> std::io::Result<()> {
    ...
    for (i, lines) in reader.lines().enumerate() {
        ...
        if parse_field {
            ...
            for (j, e) in content.chars().enumerate() {
                match e {
                    '#' => {
                        row.push(Entry2::Wall);
                        row.push(Entry2::Wall)
                    }
                    'O' => {
                        row.push(Entry2::BoxLeft);
                        row.push(Entry2::BoxRight);
                    }
                    '.' => {
                        row.push(Entry2::Empty);
                        row.push(Entry2::Empty);
                    }
                    '@' => {
                        robot.x = j as i8 * 2;
    ...
    for ((y, x), entry) in grid.indexed_iter() {
        if matches!(entry, Entry2::BoxLeft) {
            counter += (y * 100) + x;
        }
    }
